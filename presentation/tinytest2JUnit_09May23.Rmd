...
* Testing lenguage structure similar to testthat (expect_*)
...
* Convert tinytest test's results ouptut to JUnit xml: 
...
*  `test_package`: test a package during R CMD check -> causes pipeline to fail
   on fail of any test
*  `test_all`: runs tests interactively -> causes pipeline to fail on fail of
   any test
* **`run_test_dir` is the only funcion that stores test results, without exiting
  Jenkins pipeline**
...
* sets up testing framework:
  * folder in `"inst/tinytest/"` that contains expample test file
...
::: {.centered} ![](img/test_structure.png){width=1000px}
